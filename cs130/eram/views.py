...
class Item_Threads(threading.Thread):
    def __init__(self, module_list, search_term, search_mode):
    ...
    def run(self):
        for review_module in self.module_list:
            (score, number_reviews, query) = query_review_module_by_title(review_module, self.search_term, self.search_mode)
...
def search(request):
	...
	counter = 0
	    ...
	    #Start the threads
	    item_threads.append(Item_Threads(module_list, search_term, search_mode))
	    ...
	    for t in item_threads:
	        t.join()
	        item_scores.append(t.get_scores())
